...
def get_api_key():
    ...
    try:
        ...
        response = requests.post('https://api.ebulksms.com:8080/getapikey.json', json=json_array)
...
def send_sms():
    ...
    try:
        ...
        response = requests.post("https://api.ebulksms.com:8080/sendsms.json", json=sms_payload)
        ...
        if response.status_code == 200:
            ...
            if response_data.get("status") == "SUCCESS":
                ...
                print(f"Failed to send SMS. Error message: {response_data.get('message')}")
        else:
            print(f"Request failed with status code: {response.status_code}")
    except Exception as e:
        print(f"An error occurred: {str(e)}")
